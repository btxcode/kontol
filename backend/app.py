...
def start_scan():
    ...
    thread = threading.Thread(
        ...
        args=(target_domain, company_name)
    )
    thread.start()
    
    return jsonify({
        'message': 'Scan initiated. Check the status endpoint for progress.'
    }), 202
...
def get_scan_status(scan_id):
    ...
    try:
        ...
        cursor.execute("SELECT status, scan_datetime FROM scans WHERE id = %s", (scan_id,))
...
def get_dates(company):
    ...
    try:
        ...
        """, (company_id,))
        dates = [row['scan_date'].strftime('%Y%m%d') for row in cursor.fetchall()]
        cursor.close()
        connection.close()
...
def get_times(company, date):
    ...
    try:
        ...
        cursor.execute("""
            SELECT scan_time FROM scans
            WHERE company_id = %s AND scan_date = %s
            ORDER BY scan_time DESC
        """, (company_id, scan_date))
        times = []
        for row in cursor.fetchall():
            if row['scan_time']:
                scan_time_value = row['scan_time']
                # Handle timedelta or time object
                if hasattr(scan_time_value, 'total_seconds'):
                    # Convert timedelta to time
                    total_seconds = int(scan_time_value.total_seconds())
                    hours = total_seconds // 3600
                    minutes = (total_seconds % 3600) // 60
                    seconds = total_seconds % 60
                    scan_time_str = f"{hours:02d}{minutes:02d}"
                else:
                    scan_time_str = scan_time_value.strftime('%H%M')
...
def get_results(company, date, time):
    connection = get_db_connection()
    if connection is None:
        return jsonify({'data': [], 'statistics': { 'critical': 0, 'high': 0, 'medium': 0, 'low': 0, 'info': 0 }})
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.execute("SELECT id FROM companies WHERE name = %s", (company,))
        company_row = cursor.fetchone()
        ...
        # Match scan_time ignoring seconds by using LIKE pattern
        scan_time_pattern = scan_time.strftime('%H:%M') + '%'
        cursor.execute("""
            SELECT id FROM scans
            WHERE company_id = %s AND scan_date = %s AND TIME_FORMAT(scan_time, '%H:%i') LIKE %s
        """, (company_id, scan_date, scan_time_pattern))
...
def get_times(company, date):
    ...
    try:
        ...
        cursor.execute("""
            ...
            ORDER BY scan_time DESC
        """, (company_id, scan_date))
        times = [row['scan_time'].strftime('%H%M') for row in cursor.fetchall()]
        cursor.close()
        connection.close()
        return jsonify({'data': times})
    except Exception as e:
...
def get_results(company, date, time):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id FROM scans
            WHERE company_id = %s AND scan_date = %s AND scan_time = %s
